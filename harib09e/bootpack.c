...
void HariMain(void) {
  ...
  while (1) {
    ...
    if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) + fifo8_status(&timerfifo) + fifo8_status(&timerfifo2) + fifo8_status(&timerfifo3) == 0) {
      ...
      //if (t != 64)
      //        t += 16;
			sheet_refresh(sht_back, 0, 16, 16, 48);
      ...
      if (mouse_decode(&mdec, (unsigned char) i) != 0) {        
        ...
        sheet_slide(sht_mouse, mx, my);
			...
			io_sti();
			putfonts8_asc(buf_back, binfo->scrnx, 0, 64, COL8_FFFFFF, "10sec");
			sheet_refresh(sht_back, 0, 64, 56, 80);
		} else if (fifo8_status(&timerfifo2) != 0) {
			i = fifo8_get(&timerfifo2);
			io_sti();
			putfonts8_asc(buf_back, binfo->scrnx, 0, 80, COL8_FFFFFF, "3sec");
			sheet_refresh(sht_back, 0, 64, 49, 96);
		} else if (fifo8_status(&timerfifo3) != 0) { // カーソル
			i = fifo8_get(&timerfifo3);
			io_sti();
			// カーソルのチラチラ表現
			if (i != 0) {
				timer_init(timer3, &timerfifo3, 0); // 0(白)
				boxfill8(buf_back, binfo->scrnx, COL8_FFFFFF, 8, 96, 15, 111);
			} else {
				timer_init(timer3, &timerfifo3, 1); // 1(青)
