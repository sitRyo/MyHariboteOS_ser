...
; VBE存在確認
		...
		MOV		AX,0x4f00
		INT		0x10
		CMP		AX,0x004f ; 戻り値 0x004fでvesaのサポートあり。詳しくはmemo #day14
		JNE		scrn320

; VBEのバージョンチェック

		MOV		AX,[ES:DI+4] ; http://oswiki.osask.jp/?VESA #VESAインフォメーション取得
		CMP		AX,0x0200
		JB		scrn320			; if (AX < 0x0200) goto scrn320

; 画面モード情報を得る

		MOV		CX,VBEMODE ; http://oswiki.osask.jp/?VESA #画面モード情報取得
		MOV		AX,0x4f01
		INT		0x10
		CMP		AX,0x004f
		JNE		scrn320
...
; 画面モードの切り替え
		...
		MOV		[SCRNY],AX
		MOV		EAX,[ES:DI+0x28]
		MOV		[VRAM],EAX ; qemuだと30日OS本と微妙に番地が違うらしい。画面モード情報が取得できるのめっちゃ楽でいい。
		JMP		keystatus
